...
Keyword = 
[
'CppAD  cppad-20160000.0: A Package for Differentiation of C++ Algorithms  ',' algorithmic automatic derivative version cppad.hpp syntax introduction example include file preprocessor symbols namespace ',
'Install  CppAD Download, Test, and Install Instructions  ',' step 1: 2: cmake 3: check 4: installation ',
'download  Download The CppAD Source Code  ',' purpose distribution directory compressed archives current version release versions monthly unix file extraction windows and testing install instructions subversion limitations stable ',
...
'checkpoint  Checkpointing Functions  ',' syntax purpose reduce memory faster recording method constructor base advector name ay sparsity size_var option algo atom_fun clear restriction example ',
...
'atomic_ctor  Atomic Function Constructor  ',' syntax atomic_user ctor_arg_list afun implementation atomic_base restrictions name sparsity examples define ',
...
'atomic_reverse  Atomic Reverse Mode  ',' syntax purpose implementation q tx ty f g h py px ok examples define use ',
'atomic_for_sparse_jac  Atomic Forward Jacobian Sparsity Patterns  ',' syntax purpose implementation q ok examples define use ',
'atomic_rev_sparse_jac  Atomic Reverse Jacobian Sparsity Patterns  ',' syntax purpose implementation q rt st ok examples define use ',
'atomic_rev_sparse_hes  Atomic Reverse Hessian Sparsity Patterns  ',' syntax purpose implementation vx q examples define use ',
...
'atomic_sparsity.cpp  Atomic Sparsity Patterns: Example and Test  ',' function pack_sparsity_enum start class definition constructor forward for_sparse_jac rev_sparse_jac rev_sparse_hes end use recording forsparse_jac ',
...
'VecAD  AD Vectors that Record Index Operations  ',' tape reference vecad<base> syntax purpose alternatives vecad<base>::reference exceptions constructor size size_t indexing example speed and memory ',
...
'base_alloc.hpp  Example AD<Base> Where Base Constructor Allocates Memory  ',' purpose include file computed assignment macro binary operator boolean class definition condexpop condexprel equalopseq identical output integer azmul ordered unary standard math erf asinh acosh atanh expm1 log1p pow numeric_limits ',
'base_require.cpp  Using a User Defined AD Base Type: Example and Test  ',' purpose ',
'base_adolc.hpp  Enable use of AD<Base> where Base is Adolc\'s adouble Type  ',' syntax example include files condexpop condexprel equalopseq identical integer azmul ordered unary standard math erf asinh acosh atanh expm1 log1p sign abs pow numeric_limits ',
...
'sparse_hessian  Sparse Hessian: Easy Driver  ',' sparsehessian syntax purpose f w row col work color_method n_sweep vectorbase vectorset restrictions vectorsize uses forward example subset ',
...
'optimize  Optimize an ADFun Object Tape  ',' sequence operations speed memory ndebug syntax purpose improvements testing efficiency atomic functions rev_sparse_jac nan checking optimization example ',
...
'team_pthread.cpp  Pthread Implementation of a Team of AD Threads  ',' pthread_exit bug in cygwin ',
'utility  Some General Purpose Utilities  ',' testing c++ concepts numerical routines miscellaneous error handler simple vector template class multi-threading memory allocation sorting indices to_string ',
...
'SimpleVector  Definition of a Simple Vector  ',' value_type [] ndebug template class requirements elements specified default constructor sizing copy and destructor assignment size resize access using example exercise ',
...
'CppAD_vector  The CppAD::vector Template Class  ',' [] push thread_alloc syntax description include capacity assignment check size return reference move semantics element access push_back push_vector output resize clear data vectorbool memory bit_per_unit type and parallel mode example exercise ',
...
'mul_level_ode.cpp  Taylor\'s Ode Solver: A Multi-Level AD Example and Test  ',' purpose solution derivative of method using source ',
'mul_level_adolc_ode.cpp  Taylor\'s Ode Solver: A Multi-Level Adolc Example and Test  ',' purpose solution derivative of method using base_adolc.hpp memory management configuration requirement source ',
...
'speed_main  Running the Speed Test Program  ',' cppad uniform_01 syntax purpose package double profile correct seed option_list onetape optimize atomic memory sparsity options boolsparsity colpack correctness results n_sweep link functions ',
...
'zdouble  zdouble: An AD Base Type With Absolute Zero  ',' deprecated 2015-09-26 syntax constructor and assignment comparison operators arithmetic standard math nan motivation general cppad requirements example ',
